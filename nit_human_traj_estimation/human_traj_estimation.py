...
try:
    from .helper import (
        ...
        track_colors, ensure_checkpoints_downloaded,
...
class HumanTrajEstimation(Node):
    def __init__(self):
        ...
        self._shot_count = 0
        # Counts caught exceptions from external-library calls (mediapipe,
        # torch) inside the per-frame processing path, so a single bad
        # frame/library hiccup logs-and-skips instead of crashing the node
        # (and taking the activate/status services down with it).
        ...
        # Auto-download whichever required file(s) are missing, mirroring
        # nit_pose_estimation's checkpoint-auto-download behavior instead of
        # Auto-download whichever required file(s) are missing, mirroring
        # nit_pose_estimation's checkpoint-auto-download behavior instead of
        # requiring the user to place files manually.
        ensure_checkpoints_downloaded(
            required_files={
                'best_model.pth': checkpoint_path,
                'pose_landmarker.task': task_path,
            },
            share_url=self.checkpoints_url,
        ...
        self.get_logger().info(f"{GREEN}TrajMamba checkpoint loaded successfully!{RESET}")
        ...
        if self.activation:
            ...
            self.get_logger().info(
                f'{GREEN}Human trajectory estimation ready and ACTIVE, '
                f'subscribing to {self.rgb_topic} / {self.depth_topic}.{RESET}'
            )
        else:
            self.get_logger().info(
                f'{GREEN}Human trajectory estimation ready but INACTIVE '
                f'(activation:=false). Call {self.activation_service_name} with '
                f'data:=true to start processing.{RESET}'
            )
        ...
        self.get_logger().info(f"{GREEN}Human Trajectory Estimation Node started.{RESET}")
        ...
        self.get_logger().info(f"{GREEN}Activation status:    {self.activation}{RESET}")
        ...
        self.get_logger().info(f"{GREEN}Device:               {self.device}{RESET}")
        if self.enable_window:
    ...
    def _declare_parameters(self):
        ...
        self.declare_parameter(
            'checkpoints_url',
            'https://cloud.ovgu.de/s/JLQQa6f2e8qb5gn',
        )
        ...
        self.declare_parameter('activation', False)
    ...
    def _read_parameters(self):
        ...
        self.checkpoints_url = gp('checkpoints_url').get_parameter_value().string_value
    ...
    def activate_subscriptions(self):
        ...
        self.active = True
        self.get_logger().info(f"{GREEN}Human trajectory estimation activated.{RESET}")
    ...
    def deactivate_subscriptions(self):
        ...
        self.active = False
        self.get_logger().info(f"{YELLOW}Human trajectory estimation deactivated.{RESET}")
    ...
    def check_diagnostics(self):
        ...
        if self.last_rgb_raw_time is None and not self.has_warned_no_rgb:
            ...
            self.get_logger().info(f"{GREEN}RGB data now being received on '{self.rgb_topic}'.{RESET}")
            self.has_warned_no_rgb = False
        ...
        if self.last_depth_raw_time is None and not self.has_warned_no_depth:
            ...
            self.get_logger().info(f"{GREEN}Depth data now being received on '{self.depth_topic}'.{RESET}")
            self.has_warned_no_depth = False
        ...
        if not self.intrinsics_ready and not self.has_warned_no_camera_info:
            ...
            self.get_logger().info(f"{GREEN}CameraInfo now being received on '{self.camera_info_topic}'.{RESET}")
            self.has_warned_no_camera_info = False
        ...
        if have_both_raw and self.last_synced_time is None and not self.has_warned_no_sync:
            ...
            self.get_logger().info(f"{GREEN}RGB/depth synchronization recovered.{RESET}")
            self.has_warned_no_sync = False

    def status_service_callback(self, request, response):
        response.success = True
        response.message = 'active' if self.active else 'inactive'
        return response
    ...
    def activate_service_callback(self, request, response):
        ...
        response.success = True
        color = GREEN if self.active else YELLOW
        self.get_logger().info(f"{color}{response.message}{RESET}")
        return response
    ...
    def predict_track(self, track: PersonTrack):
        ...
        try:
            ...
            self.get_logger().error(
                f"TrajMamba prediction failed for track {track.id}, skipping it "
                f"({self._prediction_errors} total prediction errors so far): {exc}"
            )
            return None, False
    ...
    def synced_callback(self, rgb_msg, depth_msg):
        ...
        # A single bad frame (e.g. a mediapipe/protobuf/native-library
        # exception) must not take the whole node down — log it, skip this
        # frame, and keep spinning so the activate/status services (and
        # nit_smach) stay alive.
        try:
            result = self.landmarker.detect_for_video(mp_image, timestamp_ms)
        except Exception as exc:
            self._detection_errors += 1
            self.get_logger().error(
                f"Pose detection failed on this frame, skipping it "
                f"({self._detection_errors} total detection errors so far): {exc}"
            )
            return
